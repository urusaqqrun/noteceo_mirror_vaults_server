...
func TestE2E_ExportItem_EmptyName_UsesID(t *testing.T) {
	...
	expectedPath := "NOTE/筆記_f1/empty-name-id.json"
...
func TestE2E_ChildNoteUsesParentContainer(t *testing.T) {
	...
	if !fs.Exists("user1/NOTE/工作_f1/筆記A_n1.json") {
		t.Fatal("parent note json should exist")
	}
	if !fs.Exists("user1/NOTE/工作_f1/筆記A_n1/A評論_n2.json") {
...
func TestE2E_RenameParentMovesChildContainer(t *testing.T) {
	...
	if fs.Exists("user1/NOTE/工作_f1/筆記A_n1") || fs.Exists("user1/NOTE/工作_f1/筆記A_n1.json") {
		t.Fatal("old parent projection should be removed")
	}
	if !fs.Exists("user1/NOTE/工作_f1/筆記B_n1/A評論_n2.json") {
