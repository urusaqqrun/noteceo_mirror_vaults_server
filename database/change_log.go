...
// HardDeleteDuplicateSystemFolders 硬刪除每個用戶重複的 inbox/todoInbox（遷移用，完成後刪除）
func (s *PgStore) HardDeleteDuplicateSystemFolders(ctx context.Context) ([]string, error) {
	...
	for _, sys := range []struct{ name, itemType string }{
		...
		if err != nil {
			return nil, fmt.Errorf("query %s: %w", sys.name, err)
		}
		...
		for userID, folders := range userFolders {
			...
			canonical := folders[0]
			log.Printf("[HardDelete] user=%s type=%s canonical=%s (%d children), dupes=%d",
			...
			for _, dup := range folders[1:] {
				if dup.childCount > 0 {
					...
					if err != nil {
						log.Printf("[HardDelete]   move children %s->%s err: %v", dup.folderID, canonical.folderID, err)
					} else if n, _ := res.RowsAffected(); n > 0 {
						log.Printf("[HardDelete]   moved %d children from %s", n, dup.folderID)
					}
				}
				s.db.ExecContext(ctx, `DELETE FROM item_permissions WHERE item_id = $1`, dup.folderID)
				s.db.ExecContext(ctx, `DELETE FROM sync_changes WHERE item_id = $1`, dup.folderID)
				if _, err := s.db.ExecContext(ctx, `DELETE FROM base_items WHERE id = $1`, dup.folderID); err != nil {
					log.Printf("[HardDelete]   DELETE %s err: %v", dup.folderID, err)
				} else {
					log.Printf("[HardDelete]   hard deleted %s", dup.folderID)
				}
			}
